...
struct parser {
  ...
  // check if the option exists like --send without value ***NEW***
  bool get_option_exists(const std::string_view& option) const {
    if (const auto it = options_.find(option); it != options_.end())
      return true;
    return false;
    }
...
struct args {
  ...
  bool get_option_exists(const std::string_view & option_arg) const {
    return parser_.get_option_exists(option_arg);
  }
